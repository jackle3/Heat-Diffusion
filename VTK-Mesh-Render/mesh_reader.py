...
class QuadNode:
    ...
    def __init__(self, points, index):
        ...
        :param points: a list containing the points within the cell
        :param index: the index of the cell from the vtk file
        """
        self.points = points
        self.index = index
        self.children = []  # will store the connected quads in adjacency list

    def __str__(self):
        return 'index: {} --> points_of_cell: [ {} ] --> next_cells: {}'.format(
            str(self.index),
...
class MeshReader:
    ...
    def __init__(self, file_path, file_name):
        ...
        self.quad_adj = None  # will store the quads adjacency list
    ...
    def generate_quads_adjacency_list(self):
        quads_map = {idx: QuadNode(cell, idx) for idx, cell in enumerate(self.np_cell_list)}
        n = len(self.np_cell_list)
        for i in range(n-1):
            for j in range(i+1, n):
                dupes = len(set(self.np_cell_list[i]) & set(self.np_cell_list[j]))
                if dupes >= 2:
                    quads_map[i].children.append(j)
                    quads_map[j].children.append(i)
        for q_key in quads_map:
            quads_map[q_key].children = sorted(list(set(quads_map[q_key].children)))
        self.quad_adj = quads_map
    ...
    def setup(self):
        ...
        self.generate_quads_adjacency_list()  # generate the quads adjacency list
...
if __name__ == "__main__":
    ...
    # print(reader.points, '\n')
    # print(reader.cells, '\n')
    # print(reader.np_cell_list, '\n')
    # print(reader.np_points, '\n')
    ...
    for key in reader.quad_adj:
        print(reader.quad_adj[key])
